...
import numpy as np
import math
from connect4.network import encode_board_from_state
...
class AlphaZeroMCTS:
    ...
    def _winner_from_info(self, info):
    ...
    def _predict(self, board, player):
        if not self.use_cache:
            encoded = encode_board_from_state(board, player, self.env.rows, self.env.cols)
            return self.network.predict(encoded)
    ...
    def _predict_batch(self, boards, players):
        ...
        if self.use_cache:
            for i, (board, player) in enumerate(zip(boards, players)):
                ...
                if cached is not None:
                    ...
                    miss_states.append((board, player))
        else:
            miss_indices = list(range(len(boards)))
            miss_states = list(zip(boards, players))

        if miss_indices:
            encoded_batch = np.stack([
                encode_board_from_state(board, player, self.env.rows, self.env.cols)
                for board, player in miss_states
            ], axis=0)
            ...
            for idx, policy, value in zip(miss_indices, policies, values):
                ...
                if self.use_cache:
                    board, player = boards[idx], players[idx]
                    self._predict_cache[self._cache_key(board, player)] = results[idx]
    ...
    def _get_legal_actions(self, env, exclude_own_territory=True):
        """유효한 행동 인덱스 반환 (Connect4는 영토 개념 없음)"""
    ...
    def _expand(self, node, state, policy=None):
        ...
        if policy is None:
            policy, _ = self._predict(board, player)
    ...
    def _simulate(self, node, state, root=None, force_root=False):
        ...
        path = [node]
        current_state = state
        current_player = state[1]  # root의 플레이어
        ...
        while node.is_expanded and node.children:
            ...
            if done:
                # 승패 결정
                winner = self._winner_from_info(info)
                if winner is None:
                    if reward > 0:
        ...
        # === Evaluation ===
        # Value Network로 평가
        board, player, _ = current_state
        _, value = self._predict(board, player)
    ...
    def _simulate_batch(self, root, state, batch_size, force_root=False):
        ...
        policies, values = self._predict_batch(boards, players)
